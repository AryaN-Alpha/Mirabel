...
ROUTING_CASES: list[tuple[str, set[str] | None]] = [
    ...
    ("post this update to threads", {"threads"}),
    ("draft a reply on threads", {"threads"}),
    ("post this to threads and add a task to kanban", {"threads", "kanban"}),
    ...
    ("create a playlist of chill songs", {"spotify"}),
    ("what song is currently playing", {"spotify"}),
    ("skip to the next track", {"spotify"}),
    ("add this to the queue and post it to linkedin", {"spotify", "linkedin"}),
    ("generate an image of a futuristic cyberpunk city", {"media"}),
    ("create a video of waves crashing on rocks", {"media"}),
    ("generate an illustration and post it to linkedin", {"media", "linkedin"}),
    ("animate this picture and add a task to kanban", {"media", "kanban"}),
