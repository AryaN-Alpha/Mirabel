...
import { ErrorNote, EmptyState } from "../homeWidgets";
import { HorizontalShelf, MediaCard, TrackRow, artistNames, imageUrl, withPlaybackError } from "./spotifyShared";
  ...
  return (
    <div>
      ...
      {data.recently_played?.length > 0 && (
        <HorizontalShelf title="Continue Listening">
      ...
      {data.top_artists?.length > 0 && (
        <HorizontalShelf title="Your Top Artists">
      ...
      {data.top_tracks?.length > 0 && (
        <section style={{ marginTop: space[8] }}>
          <h2 style={{ fontFamily: fontHeading, fontSize: 22, color: text.base, marginBottom: space[4] }}>Your Top Tracks</h2>
      ...
      {data.playlists?.length > 0 && (
        <HorizontalShelf title="Your Playlists">
      ...
      {data.saved_albums?.length > 0 && (
        <HorizontalShelf title="From Your Library">
